...
to collect all the experimental results for all UCR datasets,
using `DatasetImporterDefault`

`python -m vibcreg.examples.ssl_eval_ucr_all --config_dataset config_dataset_filepath --config_framework config_framework_filepath --device_ids "0"`
"""
import os
...
if __name__ == "__main__":
    ...
    ucr_dataset_names = os.listdir(get_git_root().joinpath("vibcreg", "data", "UCRArchive_2018"))
    ucr_dataset_names = sorted(ucr_dataset_names)
    print('ucr_dataset_names:', ucr_dataset_names, '\n\n')
    ...
    for rand_seed in rand_seeds:
        for ucr_dataset_name in ucr_dataset_names:
            ...
            try:
                data_root = get_git_root().joinpath("vibcreg", "data", "UCRArchive_2018", ucr_dataset_name)
                df_train = pd.read_csv(data_root.joinpath(f"{ucr_dataset_name}_TRAIN.tsv"), sep='\t', header=None)
                df_test = pd.read_csv(data_root.joinpath(f"{ucr_dataset_name}_TEST.tsv"), sep='\t', header=None)
                seq_len = df_test.iloc[:, 1:].shape[1]
                ...
                print('================')
                print('Dataset:', ucr_dataset_name)
                print('df_train.shape:', df_train.shape)
                print('seq_len:', seq_len)
                print('crop_size:', crop_size)
                print('================')

                # if df_train.shape[0] < 600:
                # if df_train.shape[0] < 200:
                #     continue
                ...
                # W&B
                rl_util.init_wandb(config_dataset, config_framework, overwritten_project_name='UCR-ALL-kNN')
                rl_util.wandb_watch()
